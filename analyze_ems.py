...
def generate_synthetic_data(ticker, sector):
    ...
    fundamentals = {
        ...
        'pe': random.uniform(30.0, 120.0),
        'pb': random.uniform(4.0, 25.0),
        'dividend_yield': random.uniform(0.0, 1.0),
        'pat': random.uniform(1e8, 3e10),
        'roe': random.uniform(10.0, 30.0),
        'debt_to_equity': random.uniform(0.0, 1.2)
    }
...
def fetch_all_data():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(fetch_and_cache_ticker, ticker, sector, today_str): ticker for ticker, sector in EMS_CONSTITUENTS.items()}
